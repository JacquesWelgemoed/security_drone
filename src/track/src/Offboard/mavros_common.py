...
class MavrosCommon():
	...
	def setUp(self):
		...
		self.state = State()
		self.mav_type = NotImplemented
		
		self.sub_topics_ready = {	# Initializing topic ready booleans to false
			key:False
			for key in [
		...
		try:
			...
			rospy.loginfo("Ros services are ready")
		except rospy.ROSException:
			print("Failed to connect to ROS Services" )
		...
		self.ext_state_sub = rospy.Subscriber('mavros/extended_state', ExtendedState, self.extended_state_cb)
		...
		self.mission_wp_sub = rospy.Subscriber('mavros/mission/waypoints', WaypointList, self.mission_waypoints_cb)
	...
	def extended_state_cb(self, data):
		if self.extended_state.landed_state != data.landed_state:
			rospy.loginfo("landed state changed from {0} to {1}"
                 .format( mavutil.mavlink.enums['MAV_LANDED_STATE'][self.extended_state.landed_state].name,
	...
	def mission_waypoints_cb(self, data):
	...
	def state_cb(self, data):
		...
		if self.state.connected != data.connected:
			rospy.loginfo("connected state changed from {0} to {1}".format(self.state.connected, 
                                                                  data.connected))

		if self.state.mode != data.mode:
			rospy.loginfo("mode changed from {0} tpo {1}".format(self.state.mode, data.mode))
		...
		# We check data.connected because mavros publishes a disconnected state message on initiation 
		if not self.sub_topics_ready['state'] and data.connected: 	
			self.sub_topics_ready['state'] = True

	# Helper Methods

	def set_arming(self, arm, timeout):
		""" arm: True to arm, False to disarm, timeout(int): seconds"""
		rospy.loginfo("setting FCU arm: {0}".format(arm))
		loop_freq = 1
		rate = rospy.Rate(loop_freq)
		arm_set = False
		for i in xrange(timeout*loop_freq): 	# Loop runs for timeout amount of seconds
			if self.state.armed == arm:
				arm_set = True
				rospy.loginfo("set arm succes | arming took {0} seconds of an allowed {1}" .format(i/loop_freq,
                                                                                        timeout))
				break
			else:
				try:
	...
	def set_mode(self, mode, timeout):
		...
		for i in xrange(timeout * loop_freq):
			if self.state.mode == mode:
				mode_set = True
				rospy.loginfo("set mode successfully | changing mode took {0} seconds of an allowed {1}" 
	...
	def wait_for_topics(self, timeout):
		"""Wait for all topics that are subscribed to to be available | timeout(int): seconds"""
		...
		for i in xrange(loop_freq * timeout):
			try:
				...
				if res.success:
					...
					rospy.loginfo("MAV_TYPE received | type: {0}"
                   					.format(mavutil.mavlink.enums['MAV_TYPE'][self.mav_type].name))
					break
			except rospy.ServiceException as e:
				rospy.logerr(e)
